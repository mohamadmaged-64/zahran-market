...
export default function EditProductPage() {
  ...
  const { categories, getProduct, refreshProducts } = useStore();
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error } = await supabase.storage.from("products").upload(fileName, file, { cacheControl: "3600" });
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <span className="text-gray-900 dark:text-white font-medium">تعديل: {form.name}</span>
      </div>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">تعديل المنتج</h1>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" name="compare_price" value={form.compare_price} onChange={handleChange} className="input-field" step="0.01" />
            </div>
            <div>
